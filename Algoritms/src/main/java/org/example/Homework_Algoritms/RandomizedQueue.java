...
public class RandomizedQueue<Item> implements Iterable<Item> {
  ...
  // Решение к дз часть 2
//    Методы isEmpty() и size() имеют константу времени выполнения O(1), так как они просто возвращают хранимое количество элементов.
//
//        Метод enqueue() при добавлении элемента производит resize массива в случае необходимости, что занимает O(n) времени в худшем случае, где n - размер массива. В среднем resize происходит O(1) раз, что уже несколько лучше, чем добавление в динамический массив. Таким образом, амортизированное время выполнения enqueue() составляет O(1).
//
//        Метод dequeue() при удалении элемента также может производить resize массива в случае необходимости, что также занимает O(n) времени в худшем случае. Однако в среднем resize происходит O(1) раз, что также делает амортизированное время выполнения dequeue() равным O(1).
//
//        Метод sample() выбирает случайный элемент из массива, что занимает O(1) времени.
//
//        Метод iterator() производит создание копии массива и затем выбор элементов в случайном порядке, что занимает O(n^2) для каждого вызова итератора. Однако данный метод используется только для обхода очереди и не меняет её основного состояния, поэтому его амортизированное время выполнения также можно считать O(1).
//
//        Таким образом, эффективность методов RandomizedQueue можно оценить следующим образом:
//
//        - isEmpty() - O(1)
//        - size() - O(1)
//        - enqueue() - O(1) в среднем, O(n) в худшем случае
//        - dequeue() - O(1) в среднем, O(n) в худшем случае
//        - sample() - O(1)
//        - iterator() - O(1) амортизированно
